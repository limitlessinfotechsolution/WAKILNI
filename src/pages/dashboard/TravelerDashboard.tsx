        ...
        <div className="p-4 md:p-8 space-y-6 md:space-y-8">
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
              {[
                { to: '/bookings/new', icon: Plus, title: t.bookings.newBooking, desc: isRTL ? 'ابدأ حجز جديد' : 'Start a new booking', gradient: 'from-primary to-primary/80', shadow: 'shadow-glow-primary' },
                { to: '/beneficiaries', icon: Users, title: t.beneficiaries.addNew, desc: isRTL ? 'أضف مستفيد جديد' : 'Add a beneficiary', gradient: 'from-secondary to-secondary/80', shadow: 'shadow-glow-gold' },
                { to: '/services', icon: MapPin, title: t.nav.services, desc: isRTL ? 'تصفح الخدمات' : 'Browse services', gradient: 'from-emerald-500 to-emerald-600', shadow: '' },
              ].map((action, i) => (
                <Link key={action.to} to={action.to} className="block animate-fade-in" style={{ animationDelay: `${200 + i * 50}ms` }}>
                  <GlassCard className="h-full group">
                    <GlassCardContent className="p-4">
                      <div className="flex items-center gap-3">
                        <div className={cn(
                          'p-2.5 rounded-xl bg-gradient-to-br text-white shadow-lg transition-transform group-hover:scale-110',
                          action.gradient
                        )}>
          ...
          <div className="space-y-4">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
              {[PrayerTimeWidget, HijriDateWidget, QiblaWidget, QuranWidget, DuaWidget, TasbihWidget].map((Widget, i) => (
                <div key={i} className="animate-fade-in" style={{ animationDelay: `${350 + i * 60}ms` }}>
                  <Widget />
                </div>
              ))}
            </div>
              ...
              <CardHeader className="p-4 md:p-6 pb-2">
                <div className="flex items-center justify-between gap-2">
                  ...
                  <Button variant="outline" size="sm" asChild className="rounded-xl">
                    <Link to="/bookings">{isRTL ? 'عرض الكل' : 'View All'}</Link>
                  </Button>
                </div>
              </CardHeader>
              <CardContent className="p-4 md:p-6 pt-0">
                {recentBookings.length > 0 ? (
                  <div className="space-y-2">
                    {recentBookings.map((booking, i) => (
                      <Link key={booking.id} to={`/bookings/${booking.id}`} className="block">
                        <div className="flex items-center gap-4 p-3 rounded-xl bg-muted/30 hover:bg-muted/50 transition-all duration-200 active:scale-[0.98]">
                          ...
                          <Badge className={cn('text-white text-[10px] rounded-full', getStatusColor(booking.status || 'pending'))}>
                  ...
                  <div className="flex flex-col items-center justify-center py-10 text-center">
                    ...
                    <p className="text-sm text-muted-foreground mb-4 max-w-xs">{t.bookings.createFirst}</p>
